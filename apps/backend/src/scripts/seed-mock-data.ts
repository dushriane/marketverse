...
async function main() {
  ...
  console.log('🤖 Asking Gemini 3 Pro to generate vendor list for Kimironko...');
  ...
  if (!generatedVendors || generatedVendors.length === 0) {
    console.warn("⚠️ No data generated by AI. Using fallback data.");
    generatedVendors.push(
        { storeName: 'Kimironko Fresh Veg', description: 'Best vegetables in town', ownerName: 'Mama Keza', category: 'Vegetables', products: [{name: 'Carrots', price: 500}, {name: 'Potatoes (Kinigi)', price: 400}] },
        { storeName: 'Kigali Crafts', description: 'Handmade baskets and art', ownerName: 'Jean Paul', category: 'Crafts', products: [{name: 'Agaseke Basket', price: 15000}, {name: 'Wood Carving', price: 25000}] },
        { storeName: 'Rwanda Textiles', description: 'Kitenge fabrics', ownerName: 'Grace', category: 'Clothing', products: [{name: 'Kitenge (6 yards)', price: 12000}, {name: 'Dress Service', price: 8000}] }
    );
  }
  ...
  for (const vData of generatedVendors) {
      ...
      console.log(`✅ Created Vendor: ${vData.storeName}`);
      vendorCount++;
  }
